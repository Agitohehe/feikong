...
uint8_t NRF_SSI,NRF_SSI_CNT;//NRF????
uint16_t Nrf_Erro;


u8 _CH;
uint8_t NRF24L01_2_RXDATA[RX_PLOAD_WIDTH];//nrf24l01??????
uint8_t NRF24L01_2_TXDATA[RX_PLOAD_WIDTH];//nrf24l01???????
...
void ANO_NRF_Init(u8 model, u8 ch)
{
	...
	NRF24L01_Write_Reg(SPI_WRITE_REG+EN_AA,0x01); 													//????0????? 
	NRF24L01_Write_Reg(SPI_WRITE_REG+EN_RXADDR,0x01);											//????0????? 
	NRF24L01_Write_Reg(SPI_WRITE_REG+SETUP_RETR,0x1a);											//??????????:500us;????????:10? 2M????
	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_CH,ch);														//??RF???CHANAL
	NRF24L01_Write_Reg(SPI_WRITE_REG+RF_SETUP,0x0f); 												//??TX????,0db??,2Mbps,???????
	//Write_Reg(NRF_WRITE_REG+RF_SETUP,0x07); 												//??TX????,0db??,1Mbps,???????
	//Write_Reg(NRF_WRITE_REG+RF_SETUP,0x27); 												//??TX????,0db??,250Kbps,???????
/////////////////////////////////////////////////////////
	if(model==1)				//RX
	{
		NRF24L01_Write_Reg(SPI_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);								//????0??????? 
		NRF24L01_Write_Reg(SPI_WRITE_REG + NCONFIG, 0x0f);   		 // IRQ????????,16?CRC,???
	}
	else if(model==2)		//TX
	{
		NRF24L01_Write_Reg(SPI_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);								//????0??????? 
		NRF24L01_Write_Reg(SPI_WRITE_REG + NCONFIG, 0x0e);   		 // IRQ????????,16?CRC,???
	}
	else if(model==3)		//RX2
	{
